...
// Personal forks may intentionally un-ignore user-layer paths for backup/sync
// (see .gitignore "Personal fork" section). Upstream still requires ignore.
const gitignorePath = join(ROOT, '.gitignore');
const personalFork = existsSync(gitignorePath)
  && /Personal fork \(hpz0\): track user-layer/.test(readFileSync(gitignorePath, 'utf-8'));

if (personalFork) {
  pass('personal-fork mode — user-layer tracking is intentional; skip git-ignore enforcement');
} else {
